...
class MyApp(tk.Tk):
    ...
    def create_ui(self): 
        ...
        self.logo_label = tk.Label(self.root, image=self.logo_image, bg='#001F3F')
        ...
        self.logo = tk.Label(self.frame, text="Xlsx Exporter", bg='#ebe5e4', font=("Arial", 20))
        self.upload_button = tk.Button(self.frame, text="Upload dos Arquivos ZIP", command=lambda: self.run_in_thread(self.extract), width=20)
        
        # Cinco botões de extração (inicialmente invisíveis)
        self.button_1010 = tk.Button(self.frame, text="Extrair 1010",width=15, command=lambda: self.run_in_thread(self.extract_number,1010), state=tk.DISABLED)
        self.button_1200 = tk.Button(self.frame, text="Extrair 1200",width=15, command=lambda: self.run_in_thread(self.extract_number,1200), state=tk.DISABLED)
        self.button_2299 = tk.Button(self.frame, text="Extrair 2299", width=15,command=lambda: self.run_in_thread(self.extract_number,2299), state=tk.DISABLED)
        self.button_5001 = tk.Button(self.frame, text="Extrair 5001",width=15, command=lambda: self.run_in_thread(self.extract_number,5001), state=tk.DISABLED)
        self.button_5011 = tk.Button(self.frame, text="Extrair 5011",width=15, command=lambda: self.run_in_thread(self.extract_number,5011), state=tk.DISABLED)
        self.button_start = tk.Button(self.frame, text="Iniciar exportação",width=15, command=lambda: self.run_in_thread(self.start_export()), 
                                    #   state=tk.DISABLED
                                      )
       
        self.label_info = tk.Label(self.frame, text="1 - Suba o arquivo zip contendo os XML's✔️\n2 - Clique no botão correspondente ao tipo de evento para fazer a extração✔️\n3 - Clique no botão 'Iniciar exportação'✔️\n4 - Aguarde a exportação ser concluída.", bg='#ebe5e4') 
    ...
    def extract(self):
        self.pb.grid(row=5, column=0,columnspan=4, pady=15, )
        self.pb.start()
                    ...
                    for file_info in zip_ref.infolist():
                        for desired_file in desired_files:
                            if file_info.filename.endswith(desired_file):
                                ...
                                target_path = os.path.join(output_folder, os.path.splitext(desired_file)[0])
    ...
    def extract_number(self, number):
        ...
        for _folder in self.output_folder:
            ...
            if f'_{number}.xlsx' in os.listdir(os.path.join(_folder)):
                self.label_info["text"] = f"Arquivo {number}.xlsx encontrado na pasta 'output'."
                print(f"Arquivo {number}.xlsx encontrado na pasta 'output'")
                
            else:
                try:
                    iter_number = (arquivo_xml for arquivo_xml in os.listdir(os.path.join(_folder, str(number))) if arquivo_xml.endswith(f"{str(number)}.xml"))
                    ...
                    for val in iter_number:
                        ...
                        self.label_info["text"] = f"Extraindo para o xml {number} o arquivo \n{val}"
                    ...
                    df_final.to_excel(os.path.join(_folder, f'_{number}.xlsx'), index=False)
                    self.label_info["text"] = f"Extração concluída para o xml {number}.\n Quantidade de arquivos processados: {cont}"
                    print(f"Extração concluída para o xml {number}.\n Quantidade de arquivos processados: {cont}")
    ...
    def apagar_arquivos_gerados(self):
        ...
        for pasta in output_folders:
            ...
            for arquivo in arquivos:
                ...
                self.label_info["text"] = f"Deletandoo o arquivo {arquivo}"
                os.remove(os.path.join(pasta, arquivo))

    def start_export(self):
        tempo_init = time.time()
        doc_lists = {
            "1010": [], "1200": [], "2299": [], "5001": [], "5011": []
        }
        ...
        for doc_type, file_list in doc_lists.items():
            ...
            self.juntar_xmls(doc_type, file_list, pasta)
    ...
    def juntar_xmls(self, doc_number, file_list, destino):
        ...
        if len(file_list) == 0:
            return
        try:
            df_final = pd.concat([pd.read_excel(arquivo) for arquivo in file_list], ignore_index=True)
