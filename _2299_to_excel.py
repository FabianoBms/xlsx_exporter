...
from lendo_planilhas import extrair_2299
...
def json_to_dataframe(json_content):
    try:
        ...
        if data["DemonstrativoTrabalhador"]:
        ...
        # Explodir colunas com listas em novas linhas
            ...
            salvar_logs(f"DemonstrativoTrabalhador não encontrado no JSON: {data['IdentificadorEvento']}")
            df_geral = df_geral.drop(['DemonstrativoTrabalhador'], axis=1)
            return df_geral
    except Exception as e:
        print(f"Erro ao processar o arquivo: {e}")
        salvar_logs(f"Erro ao processar o arquivo: {e}")
        return None
...
def processar_xml(xml_path):
    try:
        ...
        json_content = extrair_2299(xml_content)
        df = json_to_dataframe(json_content)
        return df
    except Exception as e:
        print(f"Erro ao processar o arquivo {xml_path}: {e}")
        salvar_logs(f"Erro ao processar o arquivo {xml_path}: {e}") #Passar o nome do arquivo para gerar o log(para 2024)
        return None
...
if __name__ == '__main__':
    ...
    for arquivo_zip in arquivos_zip:
        lista_2299 = (xml for xml in zipfile.ZipFile(arquivo_zip).namelist() if xml.endswith('2299.xml'))
        pasta_2299  = zipfile.ZipFile(arquivo_zip).extractall('2299', members=lista_2299)  
    
    # Lista para armazenar DataFrames individuais
    dfs = []
    # Iterar sobre os arquivos XML na pasta
    # Iterate over the files in the '2299' directory
    for arquivo_xml in os.listdir('2299'):
        # Process each XML file
        if arquivo_xml.endswith('.xml'):
            xml_path = os.path.join('2299', arquivo_xml)
    ...
    df_final.to_csv(os.path.join('resultado_2299.xlsx'), index=False)